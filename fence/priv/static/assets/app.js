...
`
    ...
    _updateLocations(locations) {
      ...
      locations.forEach((loc) => {
        ...
        if (this.markers[loc.user_id]) {
          ...
          const marker = L.marker([loc.lat, loc.lng]).addTo(this.map).bindPopup(`<strong>${loc.display_name}</strong><br>${loc.time_ago}`);
  ...
  // js/app.js
  var Hooks2 = { LeafletMap: leaflet_map_default };
